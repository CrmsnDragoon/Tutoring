...
inline std::string OpToStr(aiTextureOp op) {
	std::string str;
	switch (op) {
...
inline std::string MapModeToStr(aiTextureMapMode op) {
	std::string str;
	switch (op) {
...
inline std::string TextureTypeToString(aiTextureType type) {
	switch (type) {
...
inline std::string AnimBehaviourToString(aiAnimBehaviour behaviour){
	switch (behaviour) {
...
inline void PrintSkeleton(const SceneNode* sceneNode, const int skeletonDepth = 0) {
...
inline void PrintAnimation(const aiAnimation* anim) {
...
inline void ImportMesh(Mesh& out, const aiMesh* mesh, const aiScene* scene) {
